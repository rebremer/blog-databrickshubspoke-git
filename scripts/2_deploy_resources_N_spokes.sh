...
create_spoke () {
   ...
   if ["$dbr_response" = ""]; then
      ...
      dbr_response=$(az databricks workspace create -l $LOC -n ${SPOKEDBRWORKSPACE}$1 -g ${SPOKERG}$1 --sku premium --vnet ${SPOKEVNET}$1 --public-subnet "public-subnet" --private-subnet "private-subnet" --enable-no-public-ip)
   fi
}
#
num=1
